...
<table width="700" border="0" cellspacing="0" cellpadding="0">
  <tr>
      ...
      <tr>
        <td colspan="3" align="center" valign="middle" ><table width="670" border="1" cellpadding="4" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;">
          <tr>
            ...
            <td width="108"><span class="style12">Total Sq. Ft.</span></td>
            <td width="109"><span class="style12">Mounting Charges</span></td>
            <td width="85"><span class="style12">Amount (INR)</span></td>
          </tr>
          <tr>
            <td align="left" valign="top"><span class="style1">Vinyl Mounting Charges for advertisement  for the under noted sites at - </span></td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
            <td>&nbsp;</td>
		 ...
		 // echo 'SELECT * FROM td_media_type WHERE type_id='.$Corders['media_type'];
		...
		$exppqty = explode(',',$Corders['main_price_unit_mounting']);
		...
		$expRentt = explode(',',$Corders['rent_t']);
          ...
          <tr>
            ...
            <td align="left" valign="top" class="style4"><br />
             <?php $Main_media_size2 = explode(',',$Corders['size']);
    ...
    $msize = $this->db->query("SELECT * FROM td_size WHERE size_id =".$Main_media_size2[$sz])->result_array();
            ...
            <td align="left" valign="top" class="style4"><br />
             <?php $Main_media_size21 = explode(',',$Corders['size']);
    $cntSZ1 = count($Main_media_size21)-1;
    for($s=0;$s<=$cntSZ1;$s++){
    $msize1 = $this->db->query("SELECT * FROM td_size WHERE size_id =".$Main_media_size21[$s])->result_array();

            echo $msize1[0]['note']*$expqty[$s];?><br />
              <?php } ?>
             </td>
            <td align="left" valign="top" class="style4"><br />
             <?php for($r=0;$r<=$expPCount;$r++) { ?>
             RS. @<?php echo $exppqty[$r];?>/ sq.ft.<br />
              <?php } ?>
             </td>
            <td align="right" valign="top" class="style4"><br />
                <?php $Main_media_size212 = explode(',',$Corders['size']);
    $cntSZ12 = count($Main_media_size212)-1;
    for($sm=0;$sm<=$cntSZ12;$sm++){
    $msize12 = $this->db->query("SELECT * FROM td_size WHERE size_id =".$Main_media_size212[$sm])->result_array();
	$rstot[]=$msize12[0]['note']*$expqty[$sm]*$exppqty[$sm];
             echo $msize12[0]['note']*$expqty[$sm]*$exppqty[$sm];?><br />
              <?php }?>
          ...
          <tr>
            <td colspan="5">&nbsp;</td>
            <td align="right" valign="middle"><span class="style1"><?php //echo $SumUnit[0]['UTotal'];?><?php echo array_sum($rstot);?></span></td>
          </tr>
          <tr>
            <td colspan="3" align="left" valign="middle" class="style4">(P.O. No. :  Date : )<br />
              Encl. : Photographs</td>
            <td colspan="2" class="style4"><?php $expt = explode(',',$expimplode); 
									$exp_cnt = count($expt)-1;
									if($data[0]['tax_id'] != '') {for($i=0;$i<=$exp_cnt;$i++) { $taxes = $this->db->query('SELECT * FROM td_service_charge WHERE sc_id='.$expt[$i])->result_array();$tid = $i+1;?><?php echo $taxes[0]['charge_name'];?> &nbsp;(<?php echo $taxes[0]['percent'];?>)<br /><?php } } ?><br/><?php if($SumOChrg[0]['otherTotal'] != 0) echo 'Other Charge';?>
             </td>
            <td align="right" valign="top" class="style4"><?php $expt = explode(',',$expimplode); 
									$exp_cnt = count($expt);
									if($data[0]['tax_id'] != '') {for($j=1;$j<=$exp_cnt;$j++) { $taxes = $this->db->query('SELECT SUM(tax'.$j.') as tttaxsc FROM td_order WHERE order_id IN ('.$timpid.')')->result_array();$tid = $j+1;?><?php $rsserv[]=$taxes[0]['tttaxsc'];echo $taxes[0]['tttaxsc'];?><br/><?php }} ?><br/><?php if($SumOChrg[0]['otherTotal'] != 0) echo $SumOChrg[0]['otherTotal'];?></td>
          </tr>
          <tr>
            <td colspan="4">&nbsp;</td>
            <td align="center" valign="middle" class="style1">TOTAL (INR)</td>
            <td align="right" valign="middle" class="style1"><?php $rs = round($SumChrg[0]['chrgTotal'], 2);echo array_sum($rstot)+array_sum($rsserv);?></td>
          </tr>
          <tr>
            <td height="30" colspan="6" align="right" valign="middle" class="style4"><strong>Amount in Words :</strong> <?php $r = round(array_sum($rstot), 2)+array_sum($rsserv).'.00'; $exprond = explode('.',$r);if(isset($exprond[1])){if($exprond[1] ==''||$exprond[1] =='0')$rtsd = $r.'.00'; else $rtsd = $r ;} else $rtsd = $r; echo no_to_words($rtsd);?></td>
            </tr>
        </table></td>
        </tr>
      <tr>
      ...
      <tr>
        ...
        <td height="38" align="right" valign="bottom">&nbsp;</td>
        <td height="38" align="right" valign="bottom" class="style12">For Bells Advertising Syndicates</td>
      </tr>
    </table></td>
  </tr>
</table>
