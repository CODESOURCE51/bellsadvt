        ...
        <td colspan="3" align="center" valign="middle" ><table width="670" border="1" cellpadding="4" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;">
          ...
          <tr>
            <td align="left" valign="top"><span class="style4"><strong><u><?php echo $MainM[0]['service_name'];?> -</u> </strong><br />
             <?php echo $marea[0]['area_name'];?>&nbsp;(<?php echo $marea[0]['landmark'];?>)<br /></td>
            <td align="left" valign="top" class="style4"><br />
             
              <?php if(isset($msize)) { echo $msize[0]['size']; } else echo 'NA';?><br />
              </td>
            <td align="left" valign="top" class="style4"><br />
              
              <?php echo $data[0]['quantity'];?><br />
             </td>
            <td align="left" valign="top" class="style4"><br />
              <?php echo $data[0]['unit_total'];?> p.m.<br />
             </td>
            <td align="left" valign="top" class="style4"><br />
              <?php echo date('d.m.y',strtotime($data[0]['rent_f']));?> - <?php echo date('d.m.y',strtotime($data[0]['rent_t']));?><br />
             </td>
            <td align="right" valign="top" class="style4"><br />
              <?php echo $data[0]['unit_total'];?><br />
             </td>
          </tr>
          <tr>
            <td colspan="5">&nbsp;</td>
            <td align="right" valign="middle"><span class="style1"><?php echo $data[0]['unit_total'];?></span></td>
          </tr>
          <tr>
            <td colspan="3" align="left" valign="middle" class="style4">(P.O. No. : 3190005157 Date : 25.05.16)<br />
              Encl. : Photographs</td>
            <td colspan="2" class="style4"><?php $expt = explode(',',$data[0]['tax_id']); 
									$exp_cnt = count($expt)-1;
									if($data[0]['tax_id'] != '') {for($i=0;$i<=$exp_cnt;$i++) { $taxes = $this->db->query('SELECT * FROM td_service_charge WHERE sc_id='.$expt[$i])->result_array();$tid = $i+1;?><?php echo $taxes[0]['charge_name'];?> &nbsp;(<?php echo $taxes[0]['percent'];?>)<br /><?php } } ?><br/><?php if($data[0]['other_charge'] != '') echo 'Other Charge';?>
             </td>
            <td align="right" valign="top" class="style4"><?php $expt = explode(',',$data[0]['tax_id']); 
									$exp_cnt = count($expt)-1;
									if($data[0]['tax_id'] != '') {for($i=0;$i<=$exp_cnt;$i++) { $taxes = $this->db->query('SELECT * FROM td_service_charge WHERE sc_id='.$expt[$i])->result_array();$tid = $i+1;?><?php echo $data[0]['tax'.$tid];?><br/><?php }} ?><br/><?php if($data[0]['other_charge'] != '') echo $data[0]['other_charge'];?></td>
          </tr>
          <tr>
            <td colspan="4">&nbsp;</td>
            <td align="center" valign="middle" class="style1">TOTAL (INR)</td>
            <td align="right" valign="middle" class="style1"><?php echo $data[0]['chrge_amt'];?></td>
          </tr>
          <tr>
            <td height="30" colspan="6" align="right" valign="middle" class="style4"><strong>Amount in Words :</strong> <?php echo no_to_words($data[0]['chrge_amt'].'.00');?></td>
            </tr>
        </table></td>
        </tr>
      <tr>
