...
<title>BELLS - Tax Invoice - Mounting</title>
<style type="text/css">
<!--
.style1 {
  ...
  <tr>
    <td width="700" height="1000" align="center" valign="top"><table width="680" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td colspan="3" align="center" valign="middle" ><table width="670" border="1" cellpadding="4" cellspacing="0" bordercolor="#000000" style="border-collapse:collapse;">
          <tr>
            <td width="334"><span class="style12">Location</span></td>
            <td width="32" align="center"><span class="style12">Size</span></td>
            <td width="43" align="center"><span class="style12">Qty</span></td>
            <td width="52" align="center"><span class="style12">Total Sq. Ft.</span></td>
            <td width="81" align="center"><span class="style12">Mounting Charges</span></td>
            <td width="66" align="center"><span class="style12">Amount (INR)</span></td>
          </tr>
          <tr>
		...
		$expRentt = explode(',',$Corders['rent_t']);
          ...
          <tr>
            ...
            <td align="center" valign="top" class="style4"><br />
              <?php if($Corders['package_type'] == 2){$Main_media_size2 = explode(',',$Corders['size']);
    ...
    $msize1 = $this->db->query("SELECT * FROM td_size WHERE size_id =".$Main_media_size21[$s])->result_array();
            ...
            echo $msize1[0]['note']*$expqty[$s];?><br />
              <?php } ?></td>
            <td align="center" valign="top" class="style4"> <br /> <?php for($r=0;$r<=$expPCount;$r++) { ?>
    ...
    $msize12 = $this->db->query("SELECT * FROM td_size WHERE size_id =".$Main_media_size212[$sm])->result_array();
             echo $msize12[0]['note']*$expqty[$sm]*$exppqty[$sm];?><br />
              <?php }?>
              <?php //echo $Corders['unit_total'];?><br /></td>
          </tr>
           <?php } ?>
          <tr>
            <td colspan="3">&nbsp;</td>
          ...
          <tr>
            <td colspan="3" align="left" valign="middle" class="style4">CAMPAIGN : <?php echo $Corders['main_media_title'];?><br/>(P.O. No. : <?php echo $Corders['po_no'];?>)</td>
            <td colspan="2" class="style4"><?php $expt = explode(',',$expimplode); 
									$exp_cnt = count($expt)-1;
									if($data[0]['tax_id'] != '') {$taxes = $this->db->query('SELECT * FROM td_service_charge WHERE sc_id IN('.$data[0]['tax_id'].')')->result_array();?><?php foreach($taxes as $tname) { ?><?php echo $tname['charge_name'];?> &nbsp;(<?php echo $tname['percent'];?>)<br /><?php } }?><br/><?php if($SumOChrg[0]['otherTotal'] != 0) echo 'Other Charge';?></td>
            <td align="right" valign="top" class="style4"><?php $expt = explode(',',$expimplode); 
									$exp_cnt = count($expt);
									if($data[0]['tax_id'] != '') {for($j=1;$j<=$exp_cnt;$j++) { $taxes = $this->db->query('SELECT SUM(tax'.$j.') as tttaxsc FROM td_order WHERE order_id IN ('.$data[0]['order_id'].')')->result_array();$tid = $j+1;?><?php echo round($taxes[0]['tttaxsc'],2);?><br/><?php }} ?><br/><?php if($SumOChrg[0]['otherTotal'] != 0) echo $SumOChrg[0]['otherTotal'];?></td>
          </tr>
          <tr>
            <td colspan="4">&nbsp;</td>
            <td align="center" valign="middle" class="style1">TOTAL (INR)</td>
            <td align="right" valign="middle" class="style1"><?php echo round($SumChrg[0]['chrgTotal'],2);?></td>
          </tr>
          <tr>
